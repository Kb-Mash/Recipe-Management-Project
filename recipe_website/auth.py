...
def login():
    if request.method == 'POST':
        ...
        # check if user email and password is valid - query
        user = User.query.filter_by(email=email).first()
        if user:
            if check_password_hash(user.password, password):
                flash('Logged in successfully!', category='success')
                login_user(user, remember=True) # remember - keeps user logged in until they clears session/history
...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            new_user = User(email=email, username=username, password=generate_password_hash(password1, method='pbkdf2:sha256')) # method - hashing algorithm
